...
CREATE TABLE Samochod (
    ...
    Moc_silnika number(6,1)  NOT NULL,
    Model_ID integer  NOT NULL,
    Wartosc_rynkowa number(15,2)  NOT NULL,
...
CREATE TABLE Ubezpieczenie (
    ID integer  NOT NULL,
    Nazwa_ubezpieczyciela varchar2(30)  NOT NULL,
    Rodzaj_ubezpieczenia_ID integer  NOT NULL,
...
CREATE TABLE Wypozyczenie (
    ...
    Klient_osoba_fizyczna_ID integer  NULL,
...
--Zapytanie SELECT, które pobierze wszystkie rekordy z tabeli Samochod:
SELECT * FROM Samochod;

--Pobierze wszystkie rekordy z tabeli Ubezpieczenie dla samochodu z modelem o ID równym 1:
SELECT * FROM Ubezpieczenie WHERE Samochod_ID = (SELECT ID FROM Samochod WHERE Model_ID = 1);

--Agregacja, która zwróci liczbê klientów, którzy s¹ firmami firm:
SELECT COUNT(*) AS LICZBA_KLIENTOW FROM Klient_firma;

--Zapytanie, które zwróci ³¹czn¹ wartoœæ wszystkich aut w tabeli Samochód:
SELECT SUM(Wartosc_rynkowa) AS £¹czna_wartoœæ_samochodow FROM Samochod;

--Polecenie DML UPDATE, które zmieni wartoœæ pola "Miasto" dla adresu o ID równym 1:
UPDATE Adres SET Miasto = 'Nowe miasto' WHERE ID = 1;

--Polecenie DML DELETE, które usunie wszystkie rekordy z tabeli Ubezpieczenie dla samochodu o ID równym 1:
DELETE FROM Ubezpieczenie WHERE Samochod_ID = 1;;
